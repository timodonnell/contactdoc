#!/usr/bin/env python3
"""Generate random-3-bins documents from afdb-24M with up to 5 entries per struct_cluster_id.

Phase 1: Scan metadata, select top 5 entries per cluster by pLDDT,
         randomly assign each to a round (0-4).
Phase 2: For each round, group by source shard, process with multiprocessing.
Phase 3: Shuffle within each round and write final output preserving round order.
"""
...
DST = "/home/ubuntu/protein-docs/random-3-bins-5x"
WORKERS = 64
SHUFFLE_SEED = 123
MAX_PER_CLUSTER = 5
...
def process_shard_entries(args):
    """Process specific entries from one source shard.

    args: (shard_path, list of (row_idx, round_num))
...
def main():
    ...
    # cluster_id -> list of (plddt, shard_path, row_idx)
    ...
    for path in tqdm(shard_paths, desc="Scanning"):
        ...
        for i, (cid, plddt) in enumerate(zip(cluster_ids, plddts)):
            ...
            if len(entries) < MAX_PER_CLUSTER:
                ...
                # Replace the lowest pLDDT entry if this one is better
    ...
    # round_num -> list of (shard_path, row_idx)
    ...
    del cluster_entries  # free memory
    ...
    global_shard_idx = 0  # continuous shard numbering across rounds
    ...
    for round_num in sorted(round_entries.keys()):
        ...
        # Write output shards split by train/val/test
        for split in ("train", "val", "test"):
            split_results = [r for r in all_results if r["split"] == split]
            if not split_results:
                continue

            # Write in chunks of ROWS_PER_OUTPUT_SHARD
